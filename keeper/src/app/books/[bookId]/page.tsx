  ...
  return (
    <div className="py-10">
      ...
      {(book.status === "collecting" || book.status === "drafting") && (
        <p className="mt-10 rounded-lg bg-sand/50 p-6 text-sm">
          ...
          return (
            <div key={page.id} className="group relative">
              ...
              >
                ...
                {quote && <div className="mt-2 text-xs italic text-ink/60">&ldquo;{quote.content.slice(0, 60)}&rdquo;</div>}
                {photoCount > 0 && (
                  <div className="mt-2 flex gap-1">
                    {Array.from({ length: Math.min(photoCount, 3) }).map((_, i) => (
                      <div key={i} className="h-8 w-8 rounded bg-sand" />
                    ))}
                  </div>
                )}
              </Link>
