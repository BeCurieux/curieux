  ...
  return (
    <div className="py-10">
      <div className="flex items-center justify-between">
        <Link href={`/books/${book.id}`} className="text-sm text-clay">&larr; Back</Link>
        {book.status === "review" && (
      ...
      <div className="mx-auto mt-8 max-w-2xl space-y-8">
        <div className="card flex aspect-[1/1.41] flex-col items-center justify-center text-center">
          ...
          {book.subtitle && <p className="mt-3 font-display text-xl text-ink/60">{book.subtitle}</p>}
        </div>
        {(pages ?? []).map((page: any) => (
          <div key={page.id} className="card aspect-[1/1.41] overflow-hidden">
            <div className="mb-3 text-[10px] uppercase tracking-wider text-ink/30">
            ...
            {(page.book_content_blocks ?? []).map((block: any) => (
              <div key={block.id} className="mb-3">
                ...
                {block.type === "caption" && <p className="text-xs text-ink/50">{block.content}</p>}
                {block.type === "quote" && (
                  <p className="font-display text-xl italic">&ldquo;{block.content}&rdquo;</p>
                )}
                {block.type === "photo" && (
                  <div className="flex h-40 items-center justify-center rounded-lg bg-sand text-xs text-ink/40">
                    photo
                  </div>
                )}
              </div>
