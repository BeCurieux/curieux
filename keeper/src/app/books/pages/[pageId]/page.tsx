              ...
              <form key={t.id} action={changeTemplate}>
                ...
                <button
                  className={`rounded-full px-4 py-1.5 text-xs ${
                    t.id === page.template_id ? "bg-ink text-paper" : "border border-sand hover:border-clay"
                  }`}
                >
                  {t.name}
                </button>
      ...
      <div className="mt-8 space-y-4">
        {(page.book_content_blocks ?? []).map((block: any) => (
          <div key={block.id} className="card">
            ...
            {block.type === "photo" ? (
              <div className="mt-2 flex h-32 items-center justify-center rounded-lg bg-sand text-xs text-ink/50">
