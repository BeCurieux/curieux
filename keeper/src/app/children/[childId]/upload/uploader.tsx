    ...
    <div>
      <div
        ...
        className={`cursor-pointer rounded-2xl border-2 border-dashed p-12 text-center transition ${
          dragOver ? "border-clay bg-sand/40" : "border-sand bg-white"
        }`}
      >
        <p className="font-display text-lg">Drop photos here</p>
        <p className="mt-1 text-sm text-ink/50">or tap to choose from your device</p>
        <input
          ref={inputRef}
          type="file"
          accept="image/*"
      ...
      {items.length > 0 && (
        <div className="mt-4 text-sm">
          ...
          <ul className="mt-2 max-h-48 space-y-1 overflow-y-auto text-xs text-ink/50">
            {items.map((i, idx) => (
              <li key={`${i.name}-${idx}`}>
                {i.state === "done" && "✓"}
                {i.state === "duplicate" && "≡"}
