...
export default async function OrderPage({ params }: { params: { orderId: string } }) {
  ...
  return (
    <div className="mx-auto max-w-lg py-16">
      <Link href={`/books/${book.id}`} className="text-sm text-clay">&larr; {book.title}</Link>
          ...
          <li key={step} className="flex items-center gap-3">
            <span
              className={`flex h-8 w-8 items-center justify-center rounded-full text-xs ${
                i <= activeIndex ? "bg-ink text-paper" : "border border-sand text-ink/40"
              }`}
            >
              {i + 1}
            </span>
            <span className={i <= activeIndex ? "" : "text-ink/40"}>{LABELS[step]}</span>
          </li>
        ))}
      </ol>
      ...
      {order.tracking_number && (
        <div className="card mt-10 text-sm">
          ...
          {order.tracking_url && (
            <>
              {" · "}
              <a className="text-clay underline" href={order.tracking_url} target="_blank" rel="noreferrer">
                Track delivery
              </a>
            </>
          )}
