      ...
      {clusters && clusters.length > 0 && (
        <section className="mt-10">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {clusters.map((c) => (
              <span key={c.id} className="rounded-full border border-sand bg-white px-4 py-1.5 text-sm">
                {c.title}
              </span>
            ))}
          </div>
        ...
        <ul className="mt-4 space-y-2">
          {(recent ?? []).map((m) => (
            <li key={m.id} className="card !p-4 text-sm">
              <span className="mr-2 rounded bg-sand px-2 py-0.5 text-xs uppercase tracking-wide">{m.type}</span>
              {m.raw_text ?? (m.type === "photo" ? "Photo" : m.type)}
              {m.memory_date && <span className="ml-2 text-ink/40">{m.memory_date}</span>}
            </li>
          ))}
          {(!recent || recent.length === 0) && (
            <li className="text-sm text-ink/50">Nothing yet — start with a few photos.</li>
          )}
        </ul>
      </section>
    </div>
