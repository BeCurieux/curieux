# Getting Keeper running on your computer
...
**Code** button → **Download ZIP**, unzip it, and open the `keeper` folder
...
2. Click **New project**.
3. Give it a name (e.g. `keeper-dev`), choose a region near you, and set a
...
2. Click **New query**.
3. Open the file `keeper/supabase/setup.sql` from the code you downloaded,
...
In the `keeper` folder, create a file named exactly **`.env.local`**
...
```powershell
cd ~\Documents\curieux\keeper
npm run seed
```

It will print a login: **demo@keeper.test** / **keeper-demo-password**.
