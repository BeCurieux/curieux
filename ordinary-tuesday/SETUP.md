# Getting Ordinary Tuesday running on your computer
...
**Code** button → **Download ZIP**, unzip it, and open the `ordinary-tuesday` folder
...
2. Click **New project**.
3. Give it a name (e.g. `ordinary-tuesday-dev`), choose a region near you, and set a
...
2. Click **New query**.
3. Open the file `ordinary-tuesday/supabase/setup.sql` from the code you downloaded,
...
In the `ordinary-tuesday` folder, create a file named exactly **`.env.local`**
...
```powershell
cd ~\Documents\curieux\ordinary-tuesday
npm run seed
```
