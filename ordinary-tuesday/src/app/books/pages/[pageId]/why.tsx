...
export function WhyIsThisHere({ sources }: { sources: { kind: string; id: string }[] }) {
  ...
  return (
    <span className="relative">
      ...
      {open && (
        <span className="absolute right-0 top-6 z-10 block w-64 rounded-lg border border-rule bg-white p-3 text-xs shadow-lg">
          ...
          <ul className="mt-1 list-disc pl-4">
            {sources.map((s, i) => (
              <li key={i}>
                ...
                <span className="text-stone"> ({s.id.slice(0, 8)})</span>
              </li>
            ))}
          </ul>
        </span>
